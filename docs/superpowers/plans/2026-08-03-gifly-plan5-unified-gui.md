...
- Go gates that MUST pass after every task that touches Go: `CGO_ENABLED=0 go build ./...`, untagged `go test ./...`, `gofmt -l .` (empty), `go vet ./...`.
- **Frontend gate, staged (important):** the migration deliberately leaves the OLD `modes/Video.svelte`/`modes/Images.svelte` in place until Task 6, but Task 1 changes their request contract (`models.ts`: `Dither` becomes a string, plus new required fields), so a full-tree `npm run build` / `npm run check` (svelte-check) CANNOT be green during Tasks 1-5 - the stale mode components would fail to typecheck, and patching those doomed files with every new field is wasted work. Therefore:
  - **Tasks 1-5 (backend + lib):** the frontend gate is `npx vitest run` (unit tests - these compile only `lib/*`, never the `.svelte` components, so they stay green) plus an isolated typecheck of the files the task changed (`npx tsc --noEmit <file>` or reasoning from the code). Do NOT require a green full-tree `npm run build` in these tasks. A full `npm run build` is EXPECTED to report errors in the stale `modes/*.svelte` during this window - that is fine and not a task failure.
  - **Tasks 6-8 (components):** `npm run build` (Vite build incl. svelte compile) and `npx vitest run` MUST both be green - Task 6 replaces/removes the stale mode components, restoring the full-tree build.
...
- [ ] **Step 6: Regenerate the TS bindings** - from `frontend/`, run `wails generate module` (regenerates `frontend/wailsjs/go/models.ts` from the Go structs). If the `wails` CLI is unavailable in this environment, hand-edit `frontend/wailsjs/go/models.ts`: in the `VideoRequest` and `ImagesRequest` classes add the new fields (`SrcWidth`, `SrcHeight`, `Aspect`, `Speed`, `Reverse`, `Boomerang`, `WebPQuality`, `Format` and change `Dither` from `boolean` to `string`) both as class properties and in the constructor's `this.X = source["X"]` assignments, mirroring the existing field pattern in that file. Verify `models.ts` is valid TypeScript with `cd frontend && npx tsc --noEmit -p tsconfig.json` (this checks `.ts` files only - it does NOT parse `.svelte`, so the stale mode components' now-wrong request literals will not surface here; that is expected per the staged frontend gate and is resolved in Task 6).

- [ ] **Step 7: Run tests + gates** - `go test ./... && go build ./... && gofmt -l . && go vet ./...` (all clean), and `cd frontend && npx vitest run && npx tsc --noEmit -p tsconfig.json` (both clean). Do NOT run `npm run build` this task (the stale `modes/*.svelte` fail to compile against the new `models.ts` until Task 6 removes them - per the staged frontend gate in Global Constraints). No Vite build runs, so `dist/.gitkeep` is untouched.
